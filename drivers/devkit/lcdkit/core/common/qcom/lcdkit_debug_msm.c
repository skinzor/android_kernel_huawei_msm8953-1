...
void lcdkit_debug_set_vsp_vsn(void *preg, int cnt)
{
    ...
    struct mdss_dsi_ctrl_pdata* pdata = platform_get_drvdata(pdev);
    ...
    rc = msm_dss_config_vreg(&pdev->dev, pdata->panel_power_data.vreg_config,
    		pdata->panel_power_data.num_vreg, 1);
    if (rc)
    {
