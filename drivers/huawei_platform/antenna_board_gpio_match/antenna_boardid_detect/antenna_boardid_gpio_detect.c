...
static ssize_t antenna_detect_show(struct device *dev,
         struct device_attribute *attr, char *buf)
{
    int det_status = 0, boardid = 0, match = 0;
...
static int antenna_boardid_detect_probe(struct platform_device *pdev)
{
    ...
    if(antenna_detect_class)
    {
        if(antenna_dev_gpio == NULL) {
            antenna_dev_gpio = device_create(antenna_detect_class, NULL, 0, NULL,"boardid");
        }

        ret = sysfs_create_link(&antenna_dev_gpio->kobj, &di->dev->kobj, "boardid_detect");
        if(ret)
        {
